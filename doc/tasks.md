...
**TODO 1**: You can open the output file in a text editor, or you can look at the control-flow graph (CFG) generated by `opt` (as you can imagine, the latter is particularly useful for visualising the high level structure of the IR - basic blocks and the relationships between them):
    ...
    opt -p dot-cfg in.ll -o /dev/null ; produces one dot file per function
    xdot cfg.f.dot

or:

    opt -p view-cfg in.ll -o /dev/null
...
**TODO 2**: The IR that you are looking at after running these commands is the most naive IR that can be generated. Most optimizations don’t work with this, but instead expect to see IR in SSA form. You can obtain this by invoking `opt` with the `mem2reg` pass.
    ...
    opt -p mem2reg in.ll -S -o out.ll

Feel free to look at the CFGs before and after if you prefer:

    opt -p view-cfg,mem2reg,view-cfg in.ll -o /dev/null
...
**TODO 1**: Get the LLVM IR corresponding to [bpk.c](https://github.com/rovka/llvm-pass-skeleton/blob/master/samples/bpk.c) and run the loop vectorization pass on it (don’t forget to pass `-Xclang -disable-O0-optnone`):
    ...
    opt -p mem2reg,loop-vectorize,view-cfg bpk.ll -o /dev/null

How was the IR transformed?

**TODO 2**: We can find out more info about what the pass is doing by using the `-pass-remarks` flag:

    opt -p mem2reg,loop-vectorize,view-cfg -pass-remarks=loop-vectorize bpk.ll -o /dev/null

You can also obtain extra info with `-debug-only=loop-vectorize`.

## Task 3 - Analysing the LLVM IR

With this exercise we’re going to start diving into the LLVM APIs. Use the code in [llvm-pass-skeleton/skeleton/](https://github.com/rovka/llvm-pass-skeleton/blob/master/skeleton/). This currently contains a skeleton for a pass.

> I'm not sure if the new pass manager supports loading passes from shared libraries, so while I figure that out you can do the following:
* copy (or link) Skeleton.cpp to the `llvm/lib/Transforms/Scalar` directory of your LLVM tree and add it to the file list in the CMakeLists.txt in that directory
* copy (or link) Skeleton.h
* add  `#include "llvm/Transforms/Scalar/Skeleton.h"` to `llvm/lib/Passes/PassBuilder.cpp`
* add `FUNCTION_PASS("skeleton", SkeletonPass())` to `llvm/lib/Passes/PassRegistry.def`
You should then be able to skip the `[-Xclang] -load libSkeletonPass.so` in the commands below (but you will have to add `-p skeleton`)..

Running the skeleton pass:

    clang -Xclang -load \
        -Xclang ~/llvm-pass-skeleton/build/skeleton/libSkeletonPass.so \
        ~/llvm-pass-skeleton/samples/fib.c -o /dev/null \
        -mllvm -debug-only=skeleton

or (after you obtain `fib.ll`)

    opt -load ~/llvm-pass-skeleton/build/skeleton/libSkeletonPass.so \
        -p skeleton -debug-only=skeleton path/to/fib.ll -o /dev/null
...
*Option 1*:
Remove the calls yourself in the same pass, after replacing their uses. The caveat here is that removing an instruction will invalidate the iterators, so you shouldn’t remove in the middle of a loop over the instructions in the basic block. Instead, you should save the instructions that you want to remove in some data structure (a `std::vector` is fine, you can also read about other data structures in the [LLVM Programmer’s Manual](https://llvm.org/docs/ProgrammersManual.html)), and then loop over that data structure and remove the instructions.

*Option 2*:
Try to use the Aggressive Dead Code Elimination pass (`-adce`) to remove the calls. Why doesn’t it work out of the box? Try to follow the code in [lib/Transforms/Scalar/ADCE.cpp](https://github.com/llvm/llvm-project/blob/main/llvm/lib/Transforms/Scalar/ADCE.cpp).
